...
namespace ChromiumWebBrowser.Core.Features.Requests.Default.Services
{
    public class DefaultRequestHandler : RequestHandler
    {
        ...
        {
            _log.Debug($"Id: {request.Identifier}; Url: {request.Url}");
            return false;
        }
    ...
    protected override bool GetAuthCredentials(IWebBrowser chromiumWebBrowser, IBrowser browser, string originUrl,
        ...
        {
            _log.Debug($"Id: {request.Identifier}; Type: {request.ResourceType}; Url: {request.Url}");
